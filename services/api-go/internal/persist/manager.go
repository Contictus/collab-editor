...
// the last snapshot. The ordered worker (F5-3) mutates these; Flush/Finalize
...
func (m *Manager) Load(ctx context.Context, docID string) (*crdt.Doc, error) {
	...
	m.mu.Lock()
	m.persisters[docID] = &Persister{docID: docID, clock: st.Clock, sinceSnapshot: st.SinceSnapshot}
	m.mu.Unlock()
	return doc, nil
}
